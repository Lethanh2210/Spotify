...
function PlayListLocation(props) {
    const { classes } = props;
    const [song1, setSong] = useState();
    const {handleSetSong, setCurrentList} = useContext(AudioSong);
    const {locationSong} = useContext(Location);

    useEffect(() =>{
        setCurrentList(locationSong);
    })
...
PlayListLocation.propTypes = {
    classes: PropTypes.object.isRequired
};

export default withStyles(styles)(PlayListLocation);
